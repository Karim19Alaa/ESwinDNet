...
class ConsoleLogger():
    ...
    def __estimate_remaining_time(self, trainer):
        ...
        if trainer.max_steps > -1:
            ...
            if self.duration_ema == -1:
                self.duration_ema = duration
            else:
                self.duration_ema = 0.999 * self.duration_ema + 0.001 * duration

            eta = format_seconds(self.duration_ema * (trainer.max_steps - trainer.global_step))
            self.train_tic()
