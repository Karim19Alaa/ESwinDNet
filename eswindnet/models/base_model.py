...
class BaseModel(pl.LightningModule):
    ...
    def setup_training(self, cfg):
        ...
        self.losses = nn.ModuleDict(
            hydra.utils.instantiate(cfg["train"]["losses"])
        ).eval()
        for p in self.losses.parameters():
            p.requires_grad = False

        self.metrics = nn.ModuleDict(
            hydra.utils.instantiate(cfg["val"]["metrics"])
        ).eval()
    ...
    def calculate_loss(self, y_hat, y, phase):
        ...
        for loss_name, loss_fn in self.losses.items():
            ...
            if isinstance(loss_dict[f"{phase}/{loss_name}"], tuple):
                loss_dict[f"{phase}/{loss_name}"] = sum(
                    loss_dict[f"{phase}/{loss_name}"]
                )
    ...
    def calculate_metrics(self, y_hat, y, phase):
        ...
        for metric_name, metric_fn in self.metrics.items():
            ...
            if isinstance(result, dict):
                metrics_dict.update(
                    {f"{phase}/{metric_name}/{k}": v for k, v in result.items()}
                )
            else:
    ...
    def configure_optimizers(self):
        optimizer = hydra.utils.instantiate(
            self.hparams.train.optim, params=self.get_bare_model().parameters()
        )
        if "scheduler" in self.hparams.train:
            scheduler = hydra.utils.instantiate(
                self.hparams.train.scheduler, optimizer=optimizer
            )
            if "monitor" in self.hparams.train:
                monitor = self.hparams.train.monitor
                return [optimizer], [
                    {"scheduler": scheduler, "monitor": monitor, "interval": "epoch"}
                ]
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        if self.trainer.global_step % self.trainer.log_every_n_steps == 0:
            self.console_logger.log_train_step(
                self.trainer, self.trainer.callback_metrics, self.optimizers()
            )
    ...
    def on_validation_epoch_end(self):
        self.console_logger.log_validation_result(
            self.trainer, self.trainer.callback_metrics
        )
        self.console_logger.train_tic()
